...
public class Asteroid extends Participant implements ShipDestroyer, AlienDestroyer
{
    ...
    private int[] speeds = { MAXIMUM_SMALL_ASTEROID_SPEED, MAXIMUM_MEDIUM_ASTEROID_SPEED,
            MAXIMUM_LARGE_ASTEROID_SPEED };

    private Clip smallDestroy = createClip("/sounds/bangSmall.wav");
    private Clip medDestroy = createClip("/sounds/bangMedium.wav");
    private Clip largeDestroy = createClip("/sounds/bangLarge.wav");

    private Clip[] sounds = { smallDestroy, medDestroy, largeDestroy };
    ...
    public Asteroid (int variety, int size, double x, double y, int speed, Controller controller)
    {
        ...
        this.size = size;
        setPosition(x, y);
        setVelocity(speeds[size], RANDOM.nextDouble() * 2 * Math.PI);
    ...
    public void collidedWith (Participant p)
    {
        if ((p instanceof AsteroidDestroyer))
        {
            ...
            playSound(sounds[size + 1]);
            for (int i = 0; i < 4; i++)
            {
                this.controller.addParticipant(new debris(getX(), getY(), 1D));
            }
            if (size >= 0)
            {
                int speed;
                if (size == 1) {
                    speed = Constants.RANDOM.nextInt(3) + 3;
                }
                else {
                    speed = Constants.RANDOM.nextInt(6) + 3;
                }

                this.controller.addParticipant(
                        new Asteroid(Constants.RANDOM.nextInt(4), size, getX(), getY(), speed, this.controller));
                this.controller.addParticipant(
                        new Asteroid(Constants.RANDOM.nextInt(4), size, getX(), getY(), speed, this.controller));
            }
            controller.asteroidDestroyed(getSize());
        }
    }
